...
public:
    ...
    //cost (calculated based on reference state)
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        // index 0 of `fg` which increments all the other values.
...
MPCSolution MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    // Initial value of the independent variables - set to 0
    Dvector vars(n_vars);
    ...
    // lower and upper limits for non-actuators set to max values( negative and positive)
    for (size_t i = 0; i < delta_start; i++) {
        vars_lowerbound[i] = -1e19;
        vars_upperbound[i] = 1e19;
    }

    // The lower and upper limits of delta are set to -25 and 25 rads
    for (size_t i = delta_start; i < a_start; i++) {
        vars_lowerbound[i] = -deg2rad25;
        vars_upperbound[i] = deg2rad25;
    }

    // lower and uppoer limits for Acceleration.
    for (size_t i = a_start; i < n_vars; i++) {
        vars_lowerbound[i] = -1.0;
        vars_upperbound[i] = 1.0;
    }

    // Lower and upper limits for the constraints - set to 0
